 ...
 *     as "aid" offsets (offsets along incoming data).
...
static SEXP new_IDS()
{
	...
	aid_offs_buf = new_IntAE(1, 0, 0);
	return R_MakeExternalPtr(aid_offs_buf, R_NilValue, R_NilValue);
}
static SEXP new_llIDS()
{
	LLongAE *aid_lloffs_buf;

	aid_lloffs_buf = new_LLongAE(1, 0, 0);
	return R_MakeExternalPtr(aid_lloffs_buf, R_NilValue, R_NilValue);
}

/* Returns IDS new length. */
static inline size_t append_aid_off_to_IDS(SEXP IDS, int aid_off)
{
	IntAE *aid_offs_buf;
	size_t IDS_len;

	aid_offs_buf = (IntAE *) R_ExternalPtrAddr(IDS);
	IDS_len = aid_offs_buf->_nelt;
	IntAE_insert_at(aid_offs_buf, IDS_len++, aid_off);
	return IDS_len;
}
static inline size_t append_aid_lloff_to_llIDS(SEXP IDS, long long aid_lloff)
{
	LLongAE *aid_lloffs_buf;
	size_t IDS_len;

	aid_lloffs_buf = (LLongAE *) R_ExternalPtrAddr(IDS);
	IDS_len = aid_lloffs_buf->_nelt;
	LLongAE_insert_at(aid_lloffs_buf, IDS_len++, aid_lloff);
	return IDS_len;
}
...
/* All buffers are made of length 'max_IDS_len' except 'sort_bufs.offs' which
   ...
   to _remove_zeros_from_leaf_vector() in merge_IDS_into_leaf_vector() below.
...
static void import_selected_one_based_positions_to_offs_buf(const int *pos,
		const int *aid_offs, int n, int maxpos, int *offs_buf)
{
	int k, m;

	for (k = 0; k < n; k++, aid_offs++, offs_buf++) {
		m = pos[*aid_offs];
...
static void import_llselected_one_based_positions_to_offs_buf(
		const int *pos,
		const long long *aid_lloffs, int n, int maxpos, int *offs_buf)
{
	int k, m;

	for (k = 0; k < n; k++, aid_lloffs++, offs_buf++) {
		m = pos[*aid_lloffs];
		if (m == NA_INTEGER || m < 1 || m > maxpos)
			error("'Mindex' contains invalid coordinates");
		*offs_buf = m - 1;
		...
		const int *aid_offs, SEXP vals)
{
	...
	_copy_Rvector_elts_from_selected_offsets(vals, aid_offs, order,
						 ans_vals);
...
static SEXP make_leaf_vector_from_selected_lloffsets(int lv_len,
		const int *order, const int *offs,
		const long long *aid_lloffs, SEXP vals)
{
	SEXP ans_offs, ans_vals, ans;
	...
	_copy_selected_ints(offs, order, lv_len, INTEGER(ans_offs));
	ans_vals = PROTECT(allocVector(TYPEOF(vals), lv_len));
	_copy_Rvector_elts_from_selected_lloffsets(vals, aid_lloffs, order,
						   ans_vals);
	ans = _new_leaf_vector(ans_offs, ans_vals);
	UNPROTECT(2);
	return ans;
...
static SEXP make_leaf_vector_from_IDS(SEXP IDS, SEXP Mindex, SEXP vals,
		int d, SortBufs *sort_bufs)
{
	IntAE *aid_offs_buf;
	int IDS_len, ans_len;

	aid_offs_buf = (IntAE *) R_ExternalPtrAddr(IDS);
	IDS_len = aid_offs_buf->_nelt;  /* guaranteed to be <= INT_MAX */
	import_selected_one_based_positions_to_offs_buf(INTEGER(Mindex),
			aid_offs_buf->elts, IDS_len, d, sort_bufs->offs);
	...
	return make_leaf_vector_from_selected_offsets(ans_len,
				...
				aid_offs_buf->elts, vals);
}
static SEXP make_leaf_vector_from_llIDS(SEXP IDS, SEXP Mindex, SEXP vals,
		int d, SortBufs *sort_bufs)
{
	LLongAE *aid_lloffs_buf;
	int IDS_len, ans_len;
	...
	aid_lloffs_buf = (LLongAE *) R_ExternalPtrAddr(IDS);
	IDS_len = aid_lloffs_buf->_nelt;  /* guaranteed to be <= INT_MAX */
	import_llselected_one_based_positions_to_offs_buf(INTEGER(Mindex),
			aid_lloffs_buf->elts, IDS_len, d, sort_bufs->offs);
	compute_offs_order(sort_bufs, IDS_len);
	ans_len = remove_offs_dups(sort_bufs->order, IDS_len, sort_bufs->offs);
	return make_leaf_vector_from_selected_lloffsets(ans_len,
				sort_bufs->order, sort_bufs->offs,
				aid_lloffs_buf->elts, vals);
}
...
{
	R_xlen_t vals_len;
	int aid_off;  /* offset along incoming data */
	//long long aid_lloff;  /* offset along incoming data */
	SEXP leaf_parent, bottom_leaf, IDS;
	int i, lv_len, ret;
	size_t IDS_len, worst_merged_len;

	vals_len = XLENGTH(vals);
	for (aid_off = 0; aid_off < vals_len; aid_off++) {
		ret = go_to_SVT_bottom(SVT, SVT0, dim, ndim,
				Mindex + aid_off, vals_len,
		...
		if (ret < 0)
			return -1;
		IDS_len = append_aid_off_to_IDS(IDS, aid_off);
		/* Update '*max_IDS_len'. */
